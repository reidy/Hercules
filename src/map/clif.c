...
const char *clif_process_chat_message(struct map_session_data *sd, const struct packet_chat_message *packet, char *out_buf, int out_buflen)
{
	...
	textlen = packet->PacketLength - 4;
#else // PACKETVER < 20151001
	textlen = packet->PacketLength - 4 - 1;
	...
	srcmessage = packet->message + namelen + 3;
...
#if PACKETVER < 20151001
	// Message should include the NUL terminator
	if (srcmessage[messagelen] != '\0' || messagelen != (int)strnlen(srcmessage, messagelen)) {
		// the declared length must match real length
		ShowWarning("clif_process_chat_message: Received malformed packet from player '%s' (length is incorrect)!\n", sd->status.name);
		return NULL;
	}
#endif

	safestrncpy(out_buf, packet->message, textlen);
...
bool clif_process_whisper_message(struct map_session_data *sd, const struct packet_whisper_message *packet, char *out_name, char *out_message, int out_messagelen)
{
	...
	if (packet->PacketLength < NAME_LENGTH + 4 + 1) {
		// 4-byte header and at least an empty string is expected
...
#if PACKETVER >= 20151001
	// Packet doesn't include a NUL terminator
	messagelen = packet->PacketLength - NAME_LENGTH - 4;
#else // PACKETVER < 20151001
	messagelen = packet->PacketLength - NAME_LENGTH - 4 - 1;
#endif // PACKETVER > 20151001
	...
	// Message should include the NUL terminator
	if (packet->message[messagelen] != '\0' || messagelen != (int)strnlen(packet->message, messagelen)) {
		// the declared length must match real length
		ShowWarning("clif_process_whisper_message: Received malformed packet from player '%s' (length is incorrect)!\n", sd->status.name);
		return false;
	}
#endif

	safestrncpy(out_name, packet->name, namelen);
	safestrncpy(out_message, packet->message, messagelen);
